	jsPsych.plugins["view"] = (function() {
		...
		plugin.trial = function(display_element, trial) {
			...
			trial_data.no_density = trial.no_density; // show prior density functions or not (true = no density/false = show density)
			trial_data.forced_strategy = trial.forced_strategy; // forced or free attention allocation (true = forced/false = free)
			if (trial.forced_strategy) {
			trial_data.forced_over = trial.forced_over ; // sequence of factors to hover over when forced
			trial_data.forced_over_time = trial.forced_over_time ; // length of time to hover over factors when forced
			} else {
			trial_data.forced_over = -1 ; // sequence of factors to hover over when forced
			trial_data.forced_over_time = -1 ; // length of time to hover over factors when forced
			}
			...
			var excessTime = trial.excess_time;
			...
			// var forced_over = [0,1,2]; // factor that is forced to hover over
			// var forced_over_time = [1000,5000,8000]; // 
			if (trial.forced_strategy) {
			var forced_over = trial.forced_over;
			var forced_over_time = trial.forced_over_time;
			var cum_forced_over_time = [];
			forced_over_time.reduce(function(a,b,i) { return cum_forced_over_time[i] = a+b; },0);
			console.log(cum_forced_over_time)
			}
			...
			if (!trial.forced_strategy) { // fc addition
			var over = nFirms + 1; // initialize free hovering over no factor
			} else {
			var over_f = 0;
			var over = forced_over[over_f];
			} // initialize forced index
			...
			function done () {
				...
				trial_data.excessTime = excessTime;
			...
			$(document.getElementById("timer")).on("click", function () {
				...
				countDown = setInterval(function() { //run this function every slice of time
					...
					if (trial.forced_strategy) { // fc addition
						...
						if (later >= trial_data.userStart + cum_forced_over_time[over_f]) { // if we have passed the viewing time of the current factor
							
							if (over_f < forced_over.length - 1) { // if we are not at the end of the forced sequence
								over_f = over_f + 1; // increment forced sequence index
								over = forced_over[over_f]; // update forced factor
							}
							
						for (var ii = 0; ii < nFirms+1; ii++) { // un-highlight all factors
						...
						if (over < nFirms+1) { // if viewing any factor
							...
							$(document.getElementById("canvas_" + over + "_density")).css("visibility","visible")

							$(document.getElementById("table_" + over + "_0")).css("background-color",colTable_0_In)
							$(document.getElementById("table_" + over + "_1")).css("background-color",colTable_In)
							$(document.getElementById("table_" + over + "_2")).css("background-color",colTable_In)
							$(document.getElementById("table_" + over + "_3")).css("background-color",colTable_In)
							$(document.getElementById("table_" + over + "_4")).css("background-color",colTable_In4)
							$(document.getElementById("table_" + over + "_5")).css("background-color",colTable_In4)
				...
				for (var i = 0; i < nFirms + 1; i++) {
					...
					if (trial.no_density) { var backCol = "rgba(0,0,255,0)"; } else { var backCol = "rgba(0,0,255,.2)"; }
