	jsPsych.plugins["forecast"] = (function() {
		...
		plugin.trial = function(display_element, trial) {
			...
			trial_data.no_density = trial.no_density;
			trial_data.feedback = trial.feedback;
			...
			if (trial.feedback) {
				var slider_visibility = "visible";
			} else {
				var slider_visibility = "hidden";
			}
			...
			if (trial.show_history) {
				...
				for (var i = 0; i < nFirms+1; i++) {
					var npr_vec = [];
					if (npr_data_n[i] && npr_data_n[i].length) {
						for (var j = 0; j < npr_data_n[i].length; j++) { npr_vec.push(npr_data_n[i][j].y) }
					}
					npr_vals[i] = npr_vec;
				}
			}
					...
					if (trial.forecast_industry || (k != 0)) {
						...
						slider.append($("<div>", {
							...
							css: {
								...
								textAlign: "center",
								lineHeight: 2,
								visibility: slider_visibility //"hidden", // fc addition
							},
							offset: {
								left: newPos,
							}
			...
			function play () {
				...
				// write npr points
				if (instructions == "" && trial.show_history) {

					var data_text = "<u>Data points</u>"+"<br>" + names[random_order_stocks[0]] + ": " + npr_vals[0].join(', ') + 
														"<br>" + names[random_order_stocks[1]] + ": " + npr_vals[1].join(', ') +
														"<br>" + names[random_order_stocks[2]] + ": " + npr_vals[2].join(', ') +
														"<br>" + names[random_order_stocks[3]] + ": " + npr_vals[3].join(', ') +
														"<br>" + names[random_order_stocks[4]] + ": " + npr_vals[4].join(', ') +
														"<br>" + names[random_order_stocks[5]] + ": " + npr_vals[5].join(', ');
					display_element.append($("<p>", {
						"id": "instructions",
						html: data_text,
						"css": {
				...
				$(".sliderwrapper").on("mousedown drag", function (event) {
					...
					if (checked == toCheck) {
						...
						$(document.getElementById("proceed")).on("click", function () { feedback(); })
